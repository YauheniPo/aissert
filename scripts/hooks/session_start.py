#!/usr/bin/env python3
"""Host-neutral SessionStart hook: inject the wiki read-plan when available.
...
WIKI_DIR = REPO_ROOT / "scripts" / "wiki"
...
def main() -> None:
    # The distributable Codex archive intentionally excludes development wiki
    # sources. Do not import them or tell an installed-plugin user to lint paths
    # that are absent from that archive.
    if not WIKI_DIR.is_dir() or not KNOWLEDGE_DIR.is_dir():
        write_context("[aissert] Plugin runtime loaded.")
        return
    ...
    if significant["significant_change"]:
        lines.append(
            "significant_change: true - run wiki maintenance (see the repository wiki rules) "
...
if __name__ == "__main__":
    try:
        ...
        # The wiki path is only available in a development checkout. Keep an
        # installed runtime plugin usable if its optional project tooling fails.
        write_context("[aissert] Plugin runtime loaded.")
